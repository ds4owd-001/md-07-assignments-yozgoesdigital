...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.\
    collection of useful libraries which work together
...
waste_data_fct <- waste_data |> 
  mutate(income_cat = factor(income_cat, level = levels_income_cat))
...
```{r}
waste_data_fct |> glimpse()

waste_data_long <- waste_data_fct |> 
  pivot_longer(food_organic_waste:yard_garden_green_waste,
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report?\
    no, I would visualise it over the two axis
...
```{r}
waste_data_long_mean |> 
  pivot_wider(values_from = mean_percent,
...
waste_data_long_mean |> 
  ...
  labs(title = "Waste Composition",
       subtitle = "mean percentage of nine waste categories displayed by income categories",
       #legend(),
       x = "mean percentage",
       y = "waste categories")
